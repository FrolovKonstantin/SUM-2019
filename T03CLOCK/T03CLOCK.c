...
LRESULT CALLBACK MyWindowFunc( HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam )
{
  ...
  static HBITMAP hBm, hBmClock, hbmadn, hbmneg;
  BITMAP bm;
  SYSTEMTIME st;
  ...
  case WM_CREATE:
    ...
    hbmadn = LoadImage(NULL, "and.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    SelectObject(hMemAnd, hbmadn);
    hMemNeg = CreateCompatibleDC(hDc);
    hbmneg = LoadImage(NULL, "neg.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    SelectObject(hMemAnd, hbmneg);
    ReleaseDC(hWnd, hDc);
    return 0;
  ...
  case WM_SIZE:
    ...
    hbmadn = CreateCompatibleBitmap(hDc, w, h);
    SelectObject(hMemAnd, hbmadn);

    hbmneg = CreateCompatibleBitmap(hDc, w, h);
    SelectObject(hMemNeg, hbmneg);
  ...
  case WM_TIMER:
    ...
    pnts3[0].x = w / 2;
    pnts3[1].x = cos((((2 * PI) / 12) * (st.wHour % 12 + st.wMinute/ 60.0)) - (PI / 2) - PI / 45) * 250 + w / 2 ;
    pnts3[2].x = cos((((2 * PI) / 12) * (st.wHour % 12 + st.wMinute/ 60.0)) - (PI / 2) - PI / 45) * 250 + w / 2;
    pnts3[3].x = cos((((2 * PI) / 12) * (st.wHour % 12 + st.wMinute/ 60.0)) - (PI / 2)) * (bm.bmWidth / 2.6) + w / 2;
    pnts3[0].y = h / 2;
    pnts3[1].y = sin((((2 * PI) / 12) * (st.wHour % 12 + st.wMinute/ 60.0)) - (PI / 2) - PI/ 45) * 250 + h / 2;
    pnts3[2].y = sin((((2 * PI) / 12) * (st.wHour % 12 + st.wMinute/ 60.0)) - (PI / 2) - PI / 45) * 250 + h / 2;
    pnts3[3].y = sin((((2 * PI) / 12) * (st.wHour % 12 + st.wMinute/ 60.0)) - (PI / 2)) * (bm.bmWidth / 2.6) + h / 2;
    ...
    GetObject(hbmadn, sizeof(bm), &bm);
    BitBlt(hMemAnd, 0, 0, w, h, hMemClockDc, 0, 0, SRCAND);
    BitBlt(hDc, 0, 0, w, h, hMemDC, 0, 0, SRCAND);
    GetObject(hbmneg, sizeof(bm), &bm);
    BitBlt(hMemNeg, 0, 0, w, h, hMemDC, 0, 0, SRCINVERT);
